...
struct True {
    template<typename ValueType>
    using eval = details::Bool<true>;
};

struct False {
    template<typename ValueType>
    using eval = details::Bool<false>;
};
...
struct Fib {
    template<typename ValueType>
    using eval = Value<ValueType, details::_Fib<I, ValueType>::val>;
};
...
struct Ref {
    template<typename ValueType, typename List>
    using eval = typename details::Find<I, List>::result;
};
...
struct Let {
    ...
    template<typename ValueType, typename List>
    using eval = typename Expr::template eval<ValueType, details::ListEnvironment<var, typename Val::template eval<ValueType, List>, List>>;
};
...
struct Eq {
    template<typename ValueType, typename List>
    using eval = typename details::_Eq<typename Left::template eval<ValueType, List>,
...
struct If {
    template<typename ValueType, typename List>
    using eval = typename details::_If<Condition::template eval<ValueType, List>::val, Then, Else>::template eval<ValueType, List>;
};
...
struct Invoke<details::_Lambda<I, Body, L>, Param> {
    static_assert(I > 0);
    template<typename ValueType, typename List>
    using eval = typename Body::template eval<ValueType, details::ListEnvironment<I, typename Param::template eval<ValueType, List>, L>>;
};

template<uint64_t I, typename Body>
struct Lambda {
    template<typename ValueType, typename List>
    using eval = details::_Lambda<I, Body, List>;
};
