...
public:
    ...
    void c() {
        for(auto a : data->elements_map)
            std::cout << (unsigned long long int)(a.first) << " | " << a.second->second << std::endl;
        std::cout << "++++LIST++++\n";
        for(auto a : data->list_of_recent_elements)
            std::cout << (unsigned long long int)(&a.first) << " | " << a.second << std::endl;
        std::cout << "-----------\n";
    }
};
